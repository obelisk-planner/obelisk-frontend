<template>
 <div class="mw6 center pa3 sans-serif">
  <h1 class="mb4">Search resources</h1>
    <input v-model="query">
    ...
    <div v-else-if="state == 'ready'">   
      <table>
        ...
        <tr v-if="results.length == 0 && page == 0">
          <td>No resources have a name containing that.</td>
        </tr>
        <div v-else-if="results.length == this.result_limit">
          <result v-for="result in results" :key="result.id" :resource="result"/>
          <tr>
            <td v-if="this.page > 0">
...
<script>
import Result_res from "@/components/Result_res.vue";
